...
interface StepTwoOTTProps {
  userId: string;
  onComplete: () => void;
}

const contentTypes = ["Series", "Films", "Documentaries", "Reality Shows", "Talk Shows", "Animated Series", "Short Films", "Standup Comedy"];
const insights = ["Viewing Patterns", "Content Demand", "Binge Behavior", "Regional Preferences", "Genre Trends"];

const StepTwoOTT = ({ userId, onComplete }: StepTwoOTTProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase.from("ott_profiles").upsert({
        user_id: user.id,
      ...
      if (error) {
        console.error("OTT profile insert error:", error);
        throw error;
      }

      toast.success("OTT profile created successfully!");
      onComplete();
    } catch (error: any) {
      console.error("OTT profile error:", error);
      toast.error(error.message || "Failed to save OTT profile. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="space-y-4">
        <h2 className="text-2xl font-semibold">📺 OTT Platform Profile</h2>
        <p className="text-muted-foreground">Tell us about your OTT platform to help you access relevant audience insights!</p>
      </div>

      <div className="space-y-4">
        <div className="space-y-2">
          <Label htmlFor="organizationName">Platform Name *</Label>
          <Input
            id="organizationName"
            value={formData.organizationName}
            onChange={(e) => setFormData({ ...formData, organizationName: e.target.value })}
            placeholder="e.g., StreamMax, ViewHub"
            required
          />
        </div>

        <div className="space-y-2">
          <Label htmlFor="organizationType">Platform Type *</Label>
          <Select
          ...
          >
            ...
            <SelectContent className="bg-popover z-50">
              <SelectItem value="subscription">Subscription-based (SVOD)</SelectItem>
              <SelectItem value="ad-supported">Ad-supported (AVOD)</SelectItem>
              <SelectItem value="transactional">Transactional (TVOD)</SelectItem>
              <SelectItem value="hybrid">Hybrid Model</SelectItem>
              <SelectItem value="free">Free Streaming</SelectItem>
            </SelectContent>
          </Select>
        </div>
        ...
        <div className="space-y-2">
          ...
          >
            ...
            <SelectContent className="bg-popover z-50">
              <SelectItem value="content-head">Content Head</SelectItem>
        ...
        <div className="space-y-2">
          <Label htmlFor="websiteLink">Platform Website *</Label>
          <Input
            id="websiteLink"
            type="url"
