...
interface StepTwoTVProps {
  userId: string;
  onComplete: () => void;
}

const contentTypes = ["Dramas", "Sitcoms", "Reality TV", "News", "Sports", "Talk Shows", "Game Shows", "Soap Operas"];
const insights = ["Viewership Ratings", "Prime Time Trends", "Demographic Preferences", "Ad Performance", "Regional Popularity"];

const StepTwoTV = ({ userId, onComplete }: StepTwoTVProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase.from("tv_profiles").upsert({
        user_id: user.id,
      ...
      if (error) {
        console.error("TV profile insert error:", error);
        throw error;
      }

      toast.success("TV profile created successfully!");
      onComplete();
    } catch (error: any) {
      console.error("TV profile error:", error);
      toast.error(error.message || "Failed to save TV profile. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="space-y-4">
        <h2 className="text-2xl font-semibold">📺 TV Network Profile</h2>
        <p className="text-muted-foreground">Tell us about your TV network to help you access relevant audience insights!</p>
      </div>

      <div className="space-y-4">
        <div className="space-y-2">
          <Label htmlFor="organizationName">Network Name *</Label>
          <Input
            id="organizationName"
            value={formData.organizationName}
            onChange={(e) => setFormData({ ...formData, organizationName: e.target.value })}
            placeholder="e.g., ABC Network, Star Plus"
            required
          />
        </div>

        <div className="space-y-2">
          <Label htmlFor="organizationType">Network Type *</Label>
          <Select
          ...
          >
            ...
            <SelectContent className="bg-popover z-50">
              <SelectItem value="broadcast">Broadcast Network</SelectItem>
              <SelectItem value="cable">Cable Network</SelectItem>
              <SelectItem value="satellite">Satellite Network</SelectItem>
              <SelectItem value="regional">Regional Channel</SelectItem>
              <SelectItem value="national">National Channel</SelectItem>
            </SelectContent>
          </Select>
        </div>
        ...
        <div className="space-y-2">
          ...
          >
            ...
            <SelectContent className="bg-popover z-50">
              <SelectItem value="programming-head">Programming Head</SelectItem>
              <SelectItem value="content-director">Content Director</SelectItem>
        ...
        <div className="space-y-2">
          <Label htmlFor="websiteLink">Network Website *</Label>
          <Input
            id="websiteLink"
            type="url"
